...
public class ClassDesignIIDriver {
	...
	public static void immutableFractionDriver() {
		...
		// the errors for the above is that you're trying to have setters for final variables
		...
		a.add(b);
		a = a.add(b); //you're returning something that you have to assign it to somewhere.
	...
	public static void passByReferenceDemo() {
		...
		System.out.println("b is :" + b);
		/*
			Passing a primitive will give you the exact value whereas passing the object requires
			you to create a new instance of it at some point if not initialized.

			You can only do that if you set it to a class-level variable, and then use that class-level variable
			in the other method. If you don't do this, then whatever you're passing in will only be available for use
			specifically for that method.

			Same thing as question number 2.

			Everything within the scope of that object when being passed into the method will be carried over
